...
__global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    for(int i = 0; i<3; i++){
        for(int j = 0; j<3; j++){
            int var = input[(filter_size * col) + (input_size * filter_size * row) + j + (input_size * i)];
            if(var > max)
                max = var;
        }
    }
...
__global__ void gemm(float *a, float *b, float *c, const float alpha, const float beta, float *output, const int input_size){
    ...
    int p = 0;
    for(p = 0; p <= input_size/TILE_WIDTH; ++p){
        // CHANGE

        if(p*TILE_WIDTH + tx < input_size  && row <input_size)
            s_a[ty][tx] = a[row * input_size + tx + p * TILE_WIDTH];
        else
            s_a[ty][tx] = 0.0;
        if(p*TILE_WIDTH + ty < input_size  && col <input_size)
            s_b[ty][tx] = b[col + input_size * ty + p * input_size * TILE_WIDTH];
        else
            s_b[ty][tx] = 0.0;
        __syncthreads();
    ...
    if(row<input_size && col <input_size)
        output[row*input_size + col] = result + c[row*input_size + col] * beta;
}
